...
constexpr decltype(auto) apply_impl(Tuple&& t, F&& f, std::index_sequence<Is...>) {
  return std::make_tuple(f(std::get<Is>(std::forward<Tuple>(t)))...);
}

template <class Tuple, class F>
constexpr decltype(auto) apply(Tuple&& t, F&& f) {
  return apply_impl(std::forward<Tuple>(t), std::forward<F>(f),
...
constexpr decltype(auto) take_front_impl(Tuple&& t, std::index_sequence<Is...>) {
