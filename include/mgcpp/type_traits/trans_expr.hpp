...
struct assert_mat_trans_expr {
  using result = typename std::enable_if<
      fold_or<is_mat_trans_expr, VariadicExpr...>::value>::type;
};

template <typename T, typename... VariadicExpr>
struct assert_mat_trans_expr_t {
  using result = typename std::
      enable_if<fold_or<is_mat_trans_expr, VariadicExpr...>::value, T>::type;
};

template <typename Expr>
struct assert_mat_trans_expr {
  using result =
      typename std::enable_if<is_mat_trans_expr<Expr>::value>> ::type;
};
