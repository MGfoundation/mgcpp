...
TEST(fft_operation, float_complex_to_complex_fwd_fft_custom_kernel)
{
    for (size_t size = 1; size <= TEST_SIZE; size *= 2)
    {
        ...
        mgcpp::mgblas_Cfft(vec.data(), result.data_mutable(), size, false);
...
TEST(fft_operation, float_complex_to_complex_inv_fft_custom_kernel)
{
    for (size_t size = 1; size <= TEST_SIZE; size *= 2)
    {
        mgcpp::device_vector<mgcpp::complex<float>> vec(size);
        ...
        mgcpp::device_vector<mgcpp::complex<float>> result(size), result2(size);
        mgcpp::mgblas_Cfft(vec.data(), result.data_mutable(), size, true);
        result = mgcpp::strict::mult(static_cast<float>(1. / size), result);
