...
void
device_matrix_test<Type>::
dimension_initializing_constructor()
{
    auto before = mgcpp::cuda_mem_get_info();
    EXPECT_TRUE(before.has_value());
    auto before_memory = before.value().first;
...
void
device_matrix_test<Type>::
third_party_matrix_construction()
{
    size_t row_dim = 5;
    size_t col_dim = 10;
...
matrix_init_from_host_data()
{
    ...
    size_t row_dim = 10;
    size_t col_dim = 10;
    mgcpp::half* data = (mgcpp::half*)malloc(
        sizeof(mgcpp::half) * row_dim * col_dim);
    ...
    EXPECT_NO_THROW(mat = mgcpp::device_matrix<mgcpp::half>(
                        {row_dim, col_dim}, data));
...
void
device_matrix_test<Type>::
reallocation_during_copy_assign()
{
    size_t row_dim = 5;
    size_t col_dim = 10;
    Type init = static_cast<Type>(7);
...
void
device_matrix_test<Type>::
no_reallocation_during_copy_assign()
{
    size_t row_dim = 5;
    size_t col_dim = 10;
    Type init = 7;
...
copy_to_host()
{
    ...
    mgcpp::half* host = (mgcpp::half*)malloc(sizeof(mgcpp::half) * row_dim * col_dim);
    EXPECT_NO_THROW(mat.copy_to_host(host));
...
move_constructor()
{
    ...
    EXPECT_NO_THROW(do {
            for (auto i = 0u; i < row_dim; ++i) {
                for (auto j = 0u; j < col_dim; ++j) {
                    EXPECT_EQ(moved.check_value(i, j),
                              Type(7));
                }
            }
        } while (false));
...
void
device_matrix_test<Type>::
matrix_zero_without_allocation_failure()
{
