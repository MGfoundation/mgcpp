...
namespace mgcpp {
cublasHandle_t thread_context::get_cublas_context(size_t device_id) {
  ...
  if (!handle) {
    ...
    handle = cublas_handle_unique_ptr(
        new_handle, [device_id](cublasHandle_t handle) {
          auto status = cuda_set_device(device_id);
          if (!status) {
            MGCPP_THROW_SYSTEM_ERROR(status.error());
          }
          std::error_code destroy_status = cublasDestroy(handle);
...
thread_context::thread_context(thread_context&& other) noexcept
    : _cublas_handle(std::move(other._cublas_handle)) {}

thread_context& thread_context::operator=(thread_context&& other) noexcept {
  _cublas_handle = std::move(other._cublas_handle);
  return *this;
}
}  // namespace mgcpp
